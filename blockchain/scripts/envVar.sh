...
setGlobals() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    ...
    export CORE_PEER_MSPCONFIGPATH=${PWD}/../artifacts/channel/crypto-config/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp
    export CORE_PEER_ADDRESS=localhost:1052
  elif [ $USING_ORG -eq 4 ]; then
    export CORE_PEER_LOCALMSPID="Org2MSP"
    export CORE_PEER_TLS_ROOTCERT_FILE=$PEER1_ORG2_CA
    export CORE_PEER_MSPCONFIGPATH=${PWD}/../artifacts/channel/crypto-config/peerOrganizations/org2.example.com/users/Admin@org2.example.com/msp
    export CORE_PEER_ADDRESS=localhost:2052
  else
    errorln "ORG Unknown"
  fi
...
setGlobalsCLI() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    ...
    export CORE_PEER_ADDRESS=peer1.org1.example.com:1052
  elif [ $USING_ORG -eq 4 ]; then
    export CORE_PEER_ADDRESS=peer1.org2.example.com:2052
  else
    errorln "ORG Unknown"
  fi
}
